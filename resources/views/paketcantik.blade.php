...
<table class="table table-hover">
  <thead>
    ...
    @foreach($data_paketcantik as $paketcantik)
  </thead>
  <tbody>
    <tr>
      <td>{{$loop->iteration}}</td>
      <td>{{$paketcantik->nama_paket}}</td>
      <td>{{$paketcantik->deskripsi}}</td>
      <td>{{$paketcantik->harga}}</td>
      <td>
      <a href="/paketcantik/{{$paketcantik->id}}/edit" class="btn btn-warning btn-sm">Edit</a>
      <a href="/paketcantik/{{$paketcantik->id}}/delete" class="btn btn-danger btn-sm " onclick= "return confirm ('Yakin mau dihapus?')">Delete</a>
      </td>
    </tr>
  </tbody>
  @endforeach
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form action= "/paketcantik/store" method="POST"  >
            @csrf
            @method('POST')
